...
"""
    ...
    lines = [
        ...
        "on_exit() {",
        '  local exit_code=$?',
        '  [[ -z "$RUNNER_SCRIPT" ]] || rm -f "$RUNNER_SCRIPT"',
        '  [[ -z "$CLEANUP_SCRIPT" ]] || rm -f "$CLEANUP_SCRIPT"',
        '  if ((exit_code != 0)); then',
        ...
        "PY",
        "}",
        'RUNNER_SCRIPT="$(mktemp "${TMPDIR:-/tmp}/visual-colab-runner.XXXXXX.py")"',
        'CLEANUP_SCRIPT="$(mktemp "${TMPDIR:-/tmp}/visual-colab-cleanup.XXXXXX.py")"',
        ...
        'colab exec -s "$SESSION_NAME" -f "$RUNNER_SCRIPT"',
